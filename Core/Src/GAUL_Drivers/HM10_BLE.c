...
void HM10BLE_ExecuteCommand(const char *command);
...
uint8_t HM10BLE_CommandTask(HM10BLE *devHM10) {
    ...
    if (isValid) {
    	HM10BLE_ExecuteCommand(command);
        return 1; // OK
    }
    return 0; // Failed
}
...
uint8_t HM10BLE_SendParameters(HM10BLE *devHM10, uint8_t *parameter) {
...
bool HM10BLE_ValidCommand(const char *received_command) {
    ...
    for (int i = 0; i < NUM_COMMANDS; i++) {
        if (strcmp(received_command, valid_commands[i]) == 0) {
            return true; // Valid
        }
    }
